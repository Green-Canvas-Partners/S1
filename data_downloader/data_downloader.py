...
from definitions.constants import API_KEY, START_DATE_GET_TICKERS, START_DATE_DATA_DOWNLOAD, END_DATE_DATA_DOWNLOAD, STOCKS_DATA_RAW_PKL, CHUNK_SIZE
from utils.custom import TickerAPI, AggregatesAPI, create_chunks


# Step 1: Fetch dates from SPY data
df = yf.download('SPY')
dates = [str(i.date()) for i in df[df.index > START_DATE_GET_TICKERS].index.tolist()]
...
for chunk in chunks:
    ticker_data = aggregates_api.fetch_multiple_tickers(tickers=chunk, from_date=START_DATE_DATA_DOWNLOAD, to_date=END_DATE_DATA_DOWNLOAD)
...
with open(STOCKS_DATA_RAW_PKL, 'wb') as f:
    pickle.dump(all_dfs, f)
print(f"All data saved to {STOCKS_DATA_RAW_PKL}.")